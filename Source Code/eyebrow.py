...
def emotion_finder(faces,frame):
    ...
    if label in ['disgust','angry','scared','sad']:
        label = 'stressed'
    else:
        label = 'not stressed'
    return label
...
def normalize_values(points,disp):
    ...
    global stress
    stress.append(int(stress_value*100))	 	
    print(stress_value)
    if stress_value>=80:
...
while(True):
    ...
    frame = imutils.resize(frame, width=840,height=850)
...
cap.release()
plt.plot(range(len(points)),stress,'ro')
